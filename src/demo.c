...
void *detect_in_thread(void *ptr)
{
    ...
    if(nboxes==0)
    {
	    xy=draw_detections(display, dets, nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes);
	    init_x[0]=*xy;
	    init_y[0]=*(xy+1);
	    init_x[1]=*xy-temp_x;
	    init_y[1]=*(xy+1)-temp_y;
	    temp_x=*xy;
	    temp_y=*(xy+1);
	    kalman2_init(&state2,init_x,init_p);
	    kalman2_init(&state2,init_y,init_p);
	    float out_x=kalman2_filter(&state2,*xy);
	    float out_y=kalman2_filter(&state2,*(xy+1));
	    free_detections(dets, nboxes);
	    printf("Prediction center: ( x= %f , y= %f )\n",out_x , out_y);
	    draw_box_width(display,out_x,*(xy+1)-12,*xy *1.1,out_y-15,display.h * .004,0,0,255);
            //real_3d(display, dets, nboxes, out_x, out_y, demo_thresh, demo_names, demo_classes);
    }
    else
    {
	xy=draw_detections(display, dets, nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes);	
	//real_3d(display, dets, nboxes, xy[0], xy[1], demo_thresh, demo_names, demo_classes);
    }
    /*
    xy=draw_detections(display, dets, nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes);
    init_x[0]=*xy;
